...
def test_bad_itxt():
    ...
    im = load(
        HEAD + chunk(b"iTXt", b"spam\0\1\1en\0Spam\0" + zlib.compress(b"egg")) + TAIL
    )
    assert im.info == {}

    im = load(
        HEAD + chunk(b"iTXt", b"spam\0\1\0en\0Spam\0" + zlib.compress(b"egg")) + TAIL
    )
    assert im.info == {"spam": "egg"}
    assert im.info["spam"].lang == "en"
    assert im.info["spam"].tkey == "Spam"
...
def test_save_p_transparent_palette(tmp_path):
...
def test_save_p_single_transparency(tmp_path):
...
def test_save_p_transparent_black(tmp_path):
...
def test_roundtrip_no_icc_profile():
    with Image.open("Tests/images/icc_profile_none.png") as im:
        assert im.info["icc_profile"] is None
...
def test_textual_chunks_after_idat():
...
def test_exif_from_jpg(tmp_path):
...
def test_exif_argument(tmp_path):
    with Image.open(TEST_PNG_FILE) as im:
        test_file = str(tmp_path / "temp.png")
