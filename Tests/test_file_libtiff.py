...
def test_g4_tiff(tmp_path):
    """Test the ordinary file path load path"""

    test_file = "Tests/images/hopper_g4_500.tif"
    with Image.open(test_file) as im:
        assert im.size == (500, 500)
        _assert_noerr(tmp_path, im)


def test_g4_large(tmp_path):
    test_file = "Tests/images/pport_g4.tif"
    with Image.open(test_file) as im:
        _assert_noerr(tmp_path, im)


def test_g4_tiff_file(tmp_path):
...
def test_g4_tiff_bytesio(tmp_path):
...
def test_g4_non_disk_file_object(tmp_path):
...
def test_g3_compression(tmp_path):
...
def test_little_endian(tmp_path):
...
def test_g4_string_info(tmp_path):
...
def test_compressions(tmp_path):
...
def xtest_bw_compression_w_rgb(tmp_path):
...
def test_crashing_metadata(tmp_path):
    # issue 1597
...
def test_page_number_x_0(tmp_path):
    # Issue 973
...
def test_fd_duplication(tmp_path):
...
def test_multipage_compression():
...
def test_save_tiff_with_jpegtables(tmp_path):
    # Arrange
    outfile = str(tmp_path / "temp.tif")
...
def test_sampleformat_not_corrupted():
