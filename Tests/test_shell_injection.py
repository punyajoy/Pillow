...
def assert_save_filename_check(tmp_path, src_img, save_func):
...
def test_load_djpeg_filename(tmp_path):
    for filename in test_filenames:
...
def test_save_cjpeg_filename(tmp_path):
    with Image.open(TEST_JPG) as im:
        assert_save_filename_check(tmp_path, im, JpegImagePlugin._save_cjpeg)


@pytest.mark.skipif(not netpbm_available(), reason="Netpbm not available")
def test_save_netpbm_filename_bmp_mode(tmp_path):
    with Image.open(TEST_GIF) as im:
        im = im.convert("RGB")
        assert_save_filename_check(tmp_path, im, GifImagePlugin._save_netpbm)


@pytest.mark.skipif(not netpbm_available(), reason="Netpbm not available")
def test_save_netpbm_filename_l_mode(tmp_path):
    with Image.open(TEST_GIF) as im:
        im = im.convert("L")
        assert_save_filename_check(tmp_path, im, GifImagePlugin._save_netpbm)
