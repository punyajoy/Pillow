...
@skip_unless_feature("zlib")
def test_truncated_with_errors():
...
@skip_unless_feature("zlib")
def test_truncated_without_errors():
    with Image.open("Tests/images/truncated_image.png") as im:
...
@skip_unless_feature("zlib")
def test_broken_datastream_with_errors():
...
def decoder():
    ...
    yield decoder
    Image.register_decoder("MOCK", None)


def test_pydecoder_setimage(decoder):
    ...
    assert decoder.state.xoff == xoff
    assert decoder.state.yoff == yoff
    assert decoder.state.xsize == xsize
    assert decoder.state.ysize == ysize

    with pytest.raises(ValueError):
        decoder.set_as_raw(b"\x00")


def test_pydecoder_extents_none(decoder):
    buf = BytesIO(b"\x00" * 255)
    ...
    assert decoder.state.xoff == 0
    assert decoder.state.yoff == 0
    assert decoder.state.xsize == 200
    assert decoder.state.ysize == 200


def test_pydecoder_negsize(decoder):
    buf = BytesIO(b"\x00" * 255)
...
def test_pydecoder_oversize(decoder):
    buf = BytesIO(b"\x00" * 255)
...
def test_pydecoder_exif_jpeg(tmp_path):
...
def test_pydecoder_exif_webp(tmp_path):
...
def test_pydecoder_exif_png(tmp_path):
...
def test_pydecoder_exif_interop():
