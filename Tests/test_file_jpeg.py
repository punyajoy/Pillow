...
def gen_random_image(size, mode="RGB"):
...
def test_icc(tmp_path):
    ...
    with Image.open(f) as im2:
        ...
        im1 = roundtrip(hopper())
        im2 = roundtrip(hopper(), icc_profile=icc_profile)
...
def test_icc_big():
    ...
    def test(n):
        ...
        im1 = roundtrip(hopper(), icc_profile=icc_profile)
...
def test_large_icc_meta(tmp_path):
...
def test_optimize():
    im1 = roundtrip(hopper())
    im2 = roundtrip(hopper(), optimize=0)
    im3 = roundtrip(hopper(), optimize=1)
    assert_image_equal(im1, im2)
...
def test_optimize_large_buffer(tmp_path):
    # https://github.com/python-pillow/Pillow/issues/148
    f = str(tmp_path / "temp.jpg")
...
def test_progressive():
    im1 = roundtrip(hopper())
    im2 = roundtrip(hopper(), progressive=False)
    im3 = roundtrip(hopper(), progressive=True)
...
def test_progressive_large_buffer(tmp_path):
    f = str(tmp_path / "temp.jpg")
    # this requires ~ 1.5x Image.MAXBLOCK
    im = Image.new("RGB", (4096, 4096), 0xFF3333)
...
def test_progressive_large_buffer_highest_quality(tmp_path):
    f = str(tmp_path / "temp.jpg")
    im = gen_random_image((255, 255))
    # this requires more bytes than pixels in the image
    im.save(f, format="JPEG", progressive=True, quality=100)


def test_progressive_cmyk_buffer():
    # Issue 2272, quality 90 cmyk image is tripping the large buffer bug.
    f = BytesIO()
    im = gen_random_image((256, 256), "CMYK")
    im.save(f, format="JPEG", progressive=True, quality=94)


def test_large_exif(tmp_path):
    # https://github.com/python-pillow/Pillow/issues/148
    f = str(tmp_path / "temp.jpg")
    im = hopper()
...
def test_progressive_compat():
    ...
    im2 = roundtrip(hopper(), progressive=0)
    im3 = roundtrip(hopper(), progression=0)  # compatibility
    assert not im2.info.get("progressive")
    assert not im2.info.get("progression")
    assert not im3.info.get("progressive")
    assert not im3.info.get("progression")

    im2 = roundtrip(hopper(), progressive=1)
    im3 = roundtrip(hopper(), progression=1)  # compatibility
    assert_image_equal(im1, im2)
    assert_image_equal(im1, im3)
...
def test_quality():
    im1 = roundtrip(hopper())
    im2 = roundtrip(hopper(), quality=50)
    assert_image(im1, im2.mode, im2.size)
    assert im1.bytes >= im2.bytes

    im3 = roundtrip(hopper(), quality=0)
...
def test_smooth():
    im1 = roundtrip(hopper())
    im2 = roundtrip(hopper(), smooth=100)
    assert_image(im1, im2.mode, im2.size)
...
def test_subsampling():
    ...
    # experimental API
    im = roundtrip(hopper(), subsampling=-1)  # default
    assert getsampling(im) == (2, 2, 1, 1, 1, 1)
    im = roundtrip(hopper(), subsampling=0)  # 4:4:4
    assert getsampling(im) == (1, 1, 1, 1, 1, 1)
    im = roundtrip(hopper(), subsampling=1)  # 4:2:2
    assert getsampling(im) == (2, 1, 1, 1, 1, 1)
    im = roundtrip(hopper(), subsampling=2)  # 4:2:0
    assert getsampling(im) == (2, 2, 1, 1, 1, 1)
    im = roundtrip(hopper(), subsampling=3)  # default (undefined)
    assert getsampling(im) == (2, 2, 1, 1, 1, 1)

    im = roundtrip(hopper(), subsampling="4:4:4")
    assert getsampling(im) == (1, 1, 1, 1, 1, 1)
    im = roundtrip(hopper(), subsampling="4:2:2")
    assert getsampling(im) == (2, 1, 1, 1, 1, 1)
    im = roundtrip(hopper(), subsampling="4:2:0")
    assert getsampling(im) == (2, 2, 1, 1, 1, 1)
    im = roundtrip(hopper(), subsampling="4:1:1")
    assert getsampling(im) == (2, 2, 1, 1, 1, 1)

    with pytest.raises(TypeError):
        roundtrip(hopper(), subsampling="1:1:1")
...
def test_quality_keep(tmp_path):
    # RGB
...
def test_truncated_jpeg_throws_oserror():
    filename = "Tests/images/truncated_jpeg.jpg"
    with Image.open(filename) as im:
...
def test_qtables(tmp_path):
    def _n_qtables_helper(n, test_file):
        ...
        with Image.open(f) as im:
            ...
            reloaded = roundtrip(im, qtables="keep")
            assert im.quantization == reloaded.quantization

    with Image.open("Tests/images/hopper.jpg") as im:
        qtables = im.quantization
        reloaded = roundtrip(im, qtables=qtables, subsampling=0)
        assert im.quantization == reloaded.quantization
        assert_image_similar(im, roundtrip(im, qtables="web_low"), 30)
        assert_image_similar(im, roundtrip(im, qtables="web_high"), 30)
        assert_image_similar(im, roundtrip(im, qtables="keep"), 30)

        # valid bounds for baseline qtable
        bounds_qtable = [int(s) for s in ("255 1 " * 32).split(None)]
        roundtrip(im, qtables=[bounds_qtable])
        ...
        assert_image_similar(
            im,
            roundtrip(im, qtables=[standard_l_qtable, standard_chrominance_qtable]),
            30,
        )
        ...
        with pytest.raises(ValueError):
            roundtrip(im, qtables="a")
        # sequence wrong length
        with pytest.raises(ValueError):
            roundtrip(im, qtables=[])
        # sequence wrong length
        with pytest.raises(ValueError):
            roundtrip(im, qtables=[1, 2, 3, 4, 5])

        # qtable entry not a sequence
        with pytest.raises(ValueError):
            roundtrip(im, qtables=[1])
        # qtable entry has wrong number of items
        with pytest.raises(ValueError):
            roundtrip(im, qtables=[[1, 2, 3, 4]])
...
def test_no_duplicate_0x1001_tag():
    # Arrange
...
def test_MAXBLOCK_scaling(tmp_path):
...
def test_dpi_exif_zero_division():
    # Arrange
...
def test_exif_x_resolution(tmp_path):
...
def test_invalid_exif_x_resolution():
...
def test_photoshop_malformed_and_multiple():
...
def test_icc_after_SOF():
    with Image.open("Tests/images/icc-after-SOF.jpg") as im:
        assert im.info["icc_profile"] == b"profile"


@pytest.mark.skipif(not is_win32(), reason="Windows only")
def test_fd_leak_win32(tmp_path):
