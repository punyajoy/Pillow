...
def test_save_setting_missing_resolution():
    b = BytesIO()
...
def test__limit_rational_float():
    from PIL.TiffImagePlugin import _limit_rational
...
def test_roundtrip_tiff_uint16(tmp_path):
...
def test_strip_planar_raw_with_overviews():
...
def test_saving_icc_profile(tmp_path):
...
def test_close_on_load_exclusive(tmp_path):
...
def test_close_on_load_nonexclusive(tmp_path):
    tmpfile = str(tmp_path / "temp.tif")
